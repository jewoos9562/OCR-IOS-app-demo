...
class LoadingdocumentViewController: UIViewController {
    ...
    func uploadImageToServer(imageData: Data?) {
        ...
        let serverURL = URL(string: "http://222.122.67.140:5012")! // 실제 서버 주소로 대체해야
                    ...
                    if
                       ...
                       let image = UIImage(data: imageData)
                       {
                        DispatchQueue.main.async {
                            // RecognitiondocumentViewController를 초기화하고 데이터를 설정한 후 전환
                            let recognitionVC = RecognitiondocumentViewController()
