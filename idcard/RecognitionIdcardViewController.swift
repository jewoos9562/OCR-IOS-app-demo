...
class RecognitionIdcardViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NSLayoutConstraint.activate([
            ...
            dateLabel.topAnchor.constraint(equalTo: idnumLabel.bottomAnchor, constant: 20)
        ])
    ...
    @objc func retryButtonTapped() {
        if let idCardVC = navigationController?.viewControllers.first(where: { $0 is idcardViewController }) {
            navigationController?.popToViewController(idCardVC, animated: true)
        }
    }
