...
class RecognitioncreditcardViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // 주민등록번호 표시
        let card_numLabel = UILabel()
        card_numLabel.text = "카드번호: \(card_num ?? "N/A")"
        card_numLabel.textColor = .white
        card_numLabel.textAlignment = .center
        card_numLabel.translatesAutoresizingMaskIntoConstraints = false
        
        view.addSubview(card_numLabel)
        
        
        NSLayoutConstraint.activate([
            card_numLabel.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            card_numLabel.topAnchor.constraint(equalTo: imageView.bottomAnchor, constant: 20),
            
            dateLabel.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            dateLabel.topAnchor.constraint(equalTo: card_numLabel.bottomAnchor, constant: 20),
...
//        ])
    ...
    @objc func retryButtonTapped() {
        if let creditcardVC = navigationController?.viewControllers.first(where: { $0 is creditcardViewController }) {
            navigationController?.popToViewController(creditcardVC, animated: true)
        }
    }
